...
# 신흥 주목작 랭킹에 포함되려면 윈도우 내 신규 리뷰가 이 이상이어야 함 (실측: 10~1000 리뷰대
# 게임은 원래 주간 자연 증가가 한 자릿수라, 절대 증가량으로만 줄세우면 베이스가 큰 게임이
# 상시 상위를 차지한다. 최소치로 순수 잡음(리뷰 1~2개)만 거르고, 정렬은 %증가율로 한다.)
